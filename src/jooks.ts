...
export class Jooks<F extends Function> {
  ...
  private mockUseReducer<R extends React.Reducer<any, any>>(
    ...
    if (!this.reducerStore.current) {
      ...
      if (this.verbose) {
        console.log(
          "Existing reducer: ",
          this.reducerStore.current.currentState
        );
      }
      this.reducerStore.current = {
        ...this.reducerStore.current,
  ...
  private mockUseCallback(
    ...
    if (existingCallback && deps !== undefined) {
      if (this.verbose) {
        console.log(
          "A callback already exists at this pointer ",
          this.callbackStore.pointer
        );
      }
      const areDepsEqual = isEqual(deps, existingCallback.deps);
      if (this.verbose) {
        console.log(
          "Did the CB dependencies change?",
          areDepsEqual,
          deps,
          existingCallback.deps
        );
      }
      if (areDepsEqual) {
        if (this.verbose) {
  ...
  private mockUseEffect(
    ...
    if (existingEffect && deps !== undefined) {
      if (this.verbose) {
        console.log(
          "An effect already exists at this pointer ",
          this.effectStore.pointer
        );
      }
  ...
  private mockUseLayoutEffect(
    ...
    if (existingEffect && deps !== undefined) {
      if (this.verbose) {
        console.log(
          "A layout effect already exists at this pointer ",
          this.layoutEffectStore.pointer
        );
      }
      const areDepsEqual = isEqual(deps, existingEffect.deps);
      if (this.verbose) {
  ...
  private mockUseRef<T>(initialValue: T) {
    ...
    if (existingRef) {
      if (this.verbose) {
        console.log(
          "A ref already exists at this pointer ",
          this.refStore.pointer
        );
      }
  ...
  private mockUseMemo<T>(
    ...
    if (existingMemo && deps !== undefined) {
      if (this.verbose) {
        console.log(
          "A memo already exists at this pointer ",
          this.memoStore.pointer
        );
      }
  ...
  private mockUseContext<T>(context: React.Context<T>): T | null {
    if (!this.contextValues.has(context)) {
      throw Error(
        `You forgot to set the context for context ${context.displayName} to useContext`
      );
    }
  ...
  private fireEffects() {
    ...
    if (this.verbose) {
      console.log(
        "Looking for effects to fire",
        this.effectStore.store,
        this.layoutEffectStore.store
      );
    }
